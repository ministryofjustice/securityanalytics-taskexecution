...
resource "null_resource" "build_image" {
  ...
  provisioner "local-exec" {
    command = "echo $ACCOUNT_ID && ${path.module}/update_docker_image.sh ${var.task_name} ${aws_ecr_repository.repo.repository_url} ${var.docker_dir} ${var.aws_region} ${var.app_name}"
  }
}
